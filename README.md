...
This repository includes [render.yaml](render.yaml), which creates a free Render web service and seeds the Chroma data during every deployment build. It uses the lightweight deterministic `hashing` embedding backend on Render to stay within the free instance's 512 MB memory limit. Local development continues to use the higher-quality sentence-transformer backend by default.
